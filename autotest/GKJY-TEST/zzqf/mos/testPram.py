...
def getOrderInfo(deduStatus=1,num=1):
    cursor = con.cursor()
    sql = "select sb_trade_num, statement_serial_num, frozen_amount from sb_settlement_records where app_id='HIS20220509DLT' and deduction_status in ({0}) and char_length(statement_serial_num) > 20 and statement_serial_num like '%607637c4%' and statement_serial_num not like '%ssss%'  and frozen_amount > 0.01 order by update_time DESC limit 0,{1}".format(deduStatus,num);
    cursor.execute(sql)
